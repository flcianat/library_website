...
def register():
    if request.method == "POST":
        ...
        openDb()
        sql = "INSERT INTO anggota (fullname, password, email, born, phonenumber, status) VALUES (%s, %s,%s, %s, %s, %s)"
        val = (fullname, password, email, born, phonenumber, status)
...
@application.route("/form-pinjam/<int:id>", methods=['GET', 'POST'])
def formpinjam(id):
    book_detail = fetch_detailbuku(id)
    return render_template("form-pinjam.html",book_detail=book_detail,email=session['email'])
